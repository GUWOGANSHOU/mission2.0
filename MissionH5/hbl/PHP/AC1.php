...
<div class="header text-center navbar-fixed-top">
	<a onclick="history.go(-1)"style="float:left; height:30pt; width:30pt; margin-top:5pt; overflow:hidden"><img src="../CSS/后退白色.png"  style="width:100%" ></a>
	<h1 style="font-size:17pt; color:#FFF; margin:0 10% ; line-height:40pt">我要认证</h1>
</div>
<div style="width:95%; height:400pt; padding-top:14pt;  margin:62pt auto; background:#FFF;">
	<div class="t t1">
    	<div  class="tu"></div>
...
-->                </form>
            ...
            <div id="showResult" style=" margin:15pt auto">
            ...
            <div style="height:32pt; width:80pt; margin:205pt auto">
            <button type="submit"  id="sahngchuan" style=" font-size:12.8pt; margin-top:35pt" class="btn btn-success  btn-block" role="button">确定</button>
...
function Cut_data(){
			...
			$.post('../handfile/ACHand.php', { StudentCardImage: imgurl,PhoneNumber:phone}, function (text, status) { alert("上传成功") });
			Email();
...
//	 $.ajax({
//	  url:"../handfile/personalDataInsertEmail.php?PhoneNumber="+$.session.get("PhoneNumber")+"&Email="+$("#Email").val(),
//	  Type:"GET",
//	  dataType:"Json",
...
//	})}  
 function ReadMsg(){
	  ...
	  success:function(data){$.each(data,function(index,comment){
		          console.log(comment.AuditState);
				  if(comment.AuditState==2){update_0(comment.PhoneNumber);}
		  	  })},
    })}
 function UpdateName(){
	 $.ajax({
	...
	function update_0(PhoneNumber){
	$.ajax({
	  url:"../handfile/updateshenhe_0.php",
	  Type:"GET",
	  dataType:"Json",
	...
	$.ajax({
		...
		dataType:"Text",
	   success:function(data){console.log(data);window.location="IndexAcceptedTask.php";},
